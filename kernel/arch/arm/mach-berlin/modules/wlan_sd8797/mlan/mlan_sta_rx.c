...
#define TDLS_CONFIRM_FIX_LEN  6
	if (len < (sizeof(EthII_Hdr_t) + 3))
		return;
	if (*(t_u8 *) (pbuf + sizeof(EthII_Hdr_t)) != TDLS_PAYLOAD_TYPE)	/* TDLS
										   payload
										   type
										   =
										   2 */
		return;
	if (*(t_u8 *) (pbuf + sizeof(EthII_Hdr_t) + 1) != TDLS_CATEGORY)	/* TDLS
										   category
										   =
										   0xc
										 */
	...
	action = *(t_u8 *) (pbuf + sizeof(EthII_Hdr_t) + 2);	/* 2= payload
								   type +
								   category */
	...
	if (action > TDLS_SETUP_CONFIRM)	/* just handle TDLS setup
	...
	if (!sta_ptr)
		return;
	if (action == TDLS_SETUP_REQUEST) {	/* TDLS setup request */
		sta_ptr->status = TDLS_SETUP_INPROGRESS;	/* TDLS_SETUP_INPROGRESS; */
	...
	if (action == TDLS_SETUP_REQUEST) {	/* setup request */
		if (len < (sizeof(EthII_Hdr_t) + TDLS_REQ_FIX_LEN))
			return;
		pos = pbuf + sizeof(EthII_Hdr_t) + 4;	/* payload 1+ category
							   1 + action 1 +dialog
							   1 */
		...
		if (len < (sizeof(EthII_Hdr_t) + TDLS_RESP_FIX_LEN))
			return;
		pos = pbuf + sizeof(EthII_Hdr_t) + 6;	/* payload 1+ category
							   1 + action 1 +dialog
							   1 +status 2 */
		sta_ptr->capability = mlan_ntohs(*(t_u16 *) pos);
		...
		if (len < (sizeof(EthII_Hdr_t) + TDLS_CONFIRM_FIX_LEN))
			return;
		pos = pbuf + sizeof(EthII_Hdr_t) + TDLS_CONFIRM_FIX_LEN;	/* payload
										   1+
										   category
										   1
										   +
