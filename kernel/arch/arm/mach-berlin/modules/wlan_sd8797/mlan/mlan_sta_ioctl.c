			   ...
			   IN pmlan_ioctl_req pioctl_req)
{
	...
	if (pmpriv->bss_type == MLAN_BSS_TYPE_STA &&
		...
		if (sta_ptr) {
			PRINTM(MCMND, "Back up TDLS peer key\n");
			sta_ptr->set_key = MTRUE;
			memcpy(pmadapter, (t_u8 *) & sta_ptr->tdls_key,
			       (t_u8 *) & sec->param.encrypt_key,
			       sizeof(sta_ptr->tdls_key));
			goto exit;
		}
	}
...
wlan_misc_cfg_ioctl(IN pmlan_adapter pmadapter, IN pmlan_ioctl_req pioctl_req)
{
	...
	case MLAN_OID_MISC_TDLS_OPER:
		...
		break;
	case MLAN_OID_MISC_TDLS_ADD_STA:
		status = wlan_misc_ioctl_tdls_add_sta(pmadapter, pioctl_req);
		break;
	case MLAN_OID_MISC_TDLS_TEARDOWN:
		status = wlan_misc_ioctl_tdls_teardown(pmadapter, pioctl_req);
		break;
