			  ...
			  IN pmlan_ioctl_req pioctl_req)
{
	...
	ret = wlan_prepare_cmd(pmpriv,
			       ...
			       HostCmd_ACT_GEN_SET,
			       0,
			       (t_void *) pioctl_req, &misc->param.tdls_oper);
 ...
 */
mlan_status
wlan_misc_ioctl_tdls_add_sta(IN pmlan_adapter pmadapter,
			     IN pmlan_ioctl_req pioctl_req)
{
	mlan_private *pmpriv = pmadapter->priv[pioctl_req->bss_index];
	mlan_status ret = MLAN_STATUS_SUCCESS;
	...
	ENTER();
	PRINTM(MIOCTL, "Add station entry for TDLS LINK\n");
	sta_ptr = wlan_add_station_entry(pmpriv, misc->param.tdls_peer_mac);
	if (sta_ptr) {
		sta_ptr->status = TDLS_SETUP_INPROGRESS;
		sta_ptr->external_tdls = MTRUE;
		wlan_hold_tdls_packets(pmpriv, misc->param.tdls_peer_mac);
	}
	LEAVE();
	return ret;
}
...
/**
 *  @brief Teardown --clear resource for tdls.
 *
 *  @param pmadapter	A pointer to mlan_adapter structure
 *  @param pioctl_req	A pointer to ioctl request buffer
 *
 ...
 */
mlan_status
wlan_misc_ioctl_tdls_teardown(IN pmlan_adapter pmadapter,
			      IN pmlan_ioctl_req pioctl_req)
{
	mlan_private *pmpriv = pmadapter->priv[pioctl_req->bss_index];
	mlan_status ret = MLAN_STATUS_SUCCESS;
	...
	ENTER();
	PRINTM(MMSG, "TDLS teardown clear all\n");
	sta_ptr = wlan_get_station_entry(pmpriv, misc->param.tdls_peer_mac);
	if (sta_ptr) {
		wlan_restore_tdls_packets(pmpriv, misc->param.tdls_peer_mac,
					  TDLS_TEAR_DOWN);
		if (sta_ptr->is_11n_enabled) {
			wlan_cleanup_reorder_tbl(pmpriv,
						 misc->param.tdls_peer_mac);
		...
		wlan_delete_station_entry(pmpriv, misc->param.tdls_peer_mac);
	}
