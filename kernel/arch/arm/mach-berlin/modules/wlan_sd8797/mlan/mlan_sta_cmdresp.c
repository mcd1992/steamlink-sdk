					  ...
					  HostCmd_DS_COMMAND * resp)
{
	...
	case WLAN_TDLS_ENABLE_LINK:
		/* TDLS enable link fail ;display error in logs */
		PRINTM(MERROR, "TDLS Enable link Failed, reason %d\n", reason);
		if (reason == TDLS_LINK_EXISTS && sta_ptr) {
			...
			memcpy(pmpriv->adapter,
			       ...
			       ptdls_oper_data->peer_mac, MLAN_MAC_ADDR_LENGTH);
			tdls_evt->reason_code =
				IEEE_80211_REASONCODE_UNSPECIFIED;
	...
	case WLAN_TDLS_DISABLE_LINK:
		/* TDLS disable link fail ;display error in logs */
		PRINTM(MERROR, "TDLS Disable link Failed, reason %d\n", reason);
		if (sta_ptr) {
			wlan_restore_tdls_packets(pmpriv,
						  ptdls_oper_data->peer_mac,
						  TDLS_TEAR_DOWN);
		   ...
		   OUT mlan_ioctl_req * pioctl_buf)
{
	...
	case WLAN_TDLS_ENABLE_LINK:
		...
		break;
	case WLAN_TDLS_DISABLE_LINK:
		PRINTM(MMSG, "TDLS: Disable link " MACSTR " success\n",
		       MAC2STR(ptdls_oper->peer_mac));
		if (sta_ptr) {
			wlan_restore_tdls_packets(pmpriv, ptdls_oper->peer_mac,
						  TDLS_TEAR_DOWN);
			...
			wlan_delete_station_entry(pmpriv, ptdls_oper->peer_mac);
		}
		if (MTRUE == wlan_is_station_list_empty(pmpriv))
			pmadapter->tdls_status = TDLS_NOT_SETUP;
		else
			     ...
			     IN t_void * pcmd_buf, IN t_void * pioctl)
{
	...
	case HostCmd_CMD_TDLS_OPERATION:
		...
		PRINTM(MCMND, "TDLS Operation enable /disable ok\n");
		break;
