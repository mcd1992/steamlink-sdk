		   ...
		   IN t_u16 cmd_action, IN t_void * pdata_buf)
{
	...
	MrvlIEtype_KeyParamSetV2_t tdls_key;
	...
	ptdls_oper->reason = wlan_cpu_to_le16(tdls_oper->reason);
	ptdls_oper->tdls_action = wlan_cpu_to_le16(tdls_oper->tdls_action);
	...
	case WLAN_TDLS_ENABLE_LINK:
		if (sta_ptr) {
			/* capability */
			memcpy(pmpriv->adapter, pos, &sta_ptr->capability,
			       sizeof(sta_ptr->capability));
			...
			Rate_tlv =
				(MrvlIEtypes_RatesParamSet_t *) (pos +
								 travel_len);
			Rate_tlv->header.type = TLV_TYPE_RATES;
			Rate_tlv->header.len =
			...
			if (sta_ptr->HTcap.ieee_hdr.element_id == HT_CAPABILITY) {
				HTcap_tlv =
					(MrvlIETypes_HTCap_t *) (pos +
								 travel_len);
				HTcap_tlv->header.type = TLV_TYPE_HT_CAP;
				HTcap_tlv->header.len =
					(t_u16) sta_ptr->HTcap.ieee_hdr.len;
				HTcap_tlv->header.len =
					wlan_cpu_to_le16(HTcap_tlv->header.len);
				memcpy(pmpriv->adapter, &HTcap_tlv->ht_cap,
				       &sta_ptr->HTcap.ht_cap,
				       sta_ptr->HTcap.ieee_hdr.len);
				travel_len += sizeof(MrvlIETypes_HTCap_t);
			}
			/* HT info */
			if (sta_ptr->HTcap.ieee_hdr.element_id == HT_OPERATION) {
				HTInfo_tlv =
					(MrvlIETypes_HTInfo_t *) (pos +
								  travel_len);
				HTInfo_tlv->header.type = TLV_TYPE_HT_INFO;
				HTInfo_tlv->header.len =
					(t_u16) sta_ptr->HTInfo.ieee_hdr.len;
				memcpy(pmpriv->adapter, &HTInfo_tlv->ht_info,
				       &sta_ptr->HTInfo.ht_info,
				       sta_ptr->HTInfo.ieee_hdr.len);
				travel_len += sizeof(MrvlIETypes_HTInfo_t);
			} else {
				HTInfo_tlv =
					(MrvlIETypes_HTInfo_t *) (pos +
								  travel_len);
				HTInfo_tlv->header.type = TLV_TYPE_HT_INFO;
				HTInfo_tlv->header.len = sizeof(HTInfo_t);
				HTInfo_tlv->ht_info.pri_chan =
					pmpriv->curr_bss_params.bss_descriptor.
					channel;
				HTInfo_tlv->ht_info.field3 = 0x5;
				HTInfo_tlv->ht_info.basic_mcs_set[0] = 0xff;
				travel_len += sizeof(MrvlIETypes_HTInfo_t);
			}
			/* 20/40 BSS co-exist */
			if (sta_ptr->BSSCO_20_40.ieee_hdr.element_id ==
				...
				BSSCo->header.len =
					(t_u16) sta_ptr->BSSCO_20_40.ieee_hdr.
					len;
				memcpy(pmpriv->adapter, &BSSCo->bss_co_2040,
				       &sta_ptr->BSSCO_20_40.bss_co_2040,
				       sta_ptr->BSSCO_20_40.ieee_hdr.len);
				travel_len += sizeof(MrvlIETypes_2040BSSCo_t);
			}
			/* Key material */
			if (sta_ptr->set_key) {
				tdls_key.type =
					wlan_cpu_to_le16(TLV_TYPE_KEY_PARAM_V2);
				tdls_key.key_idx = sta_ptr->tdls_key.key_index;
				tdls_key.key_info =
					KEY_INFO_ENABLE_KEY | KEY_INFO_TX_KEY |
					KEY_INFO_RX_KEY | KEY_INFO_UCAST_KEY |
					KEY_INFO_DEFAULT_KEY;
				tdls_key.key_info =
					wlan_cpu_to_le16(tdls_key.key_info);

				tdls_key.key_type = KEY_TYPE_ID_AES;
				tdls_key.key_params.aes.key_len =
					wlan_cpu_to_le16(sta_ptr->tdls_key.
							 key_len);
				memcpy(pmpriv->adapter,
				       tdls_key.key_params.aes.key,
				       sta_ptr->tdls_key.key_material,
				       sta_ptr->tdls_key.key_len);

				if (sta_ptr->tdls_key.
				    key_flags & (KEY_FLAG_RX_SEQ_VALID |
						 KEY_FLAG_TX_SEQ_VALID))
					memcpy(pmpriv->adapter,
					       tdls_key.key_params.aes.pn,
					       sta_ptr->tdls_key.pn,
					       SEQ_MAX_SIZE);
				memcpy(pmpriv->adapter, tdls_key.mac_addr,
				       sta_ptr->tdls_key.mac_addr,
				       MLAN_MAC_ADDR_LENGTH);
				tdls_key.length =
					wlan_cpu_to_le16(KEY_PARAMS_FIXED_LEN +
							 sizeof(aes_param));
				memcpy(pmpriv->adapter, pos + travel_len,
				       &tdls_key, sizeof(tdls_key));
				travel_len += sizeof(tdls_key);
			}
		}
		break;
