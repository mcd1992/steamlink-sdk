 ...
 */
static int
woal_tdls_oper_ioctl(moal_private * priv, u8 * peer, t_u8 action)
{
...
/**
 *  @brief TDLS add sta entry ioctl handler
 *
 *  @param priv     A pointer to moal_private structure
 *  @param peer     A pointer to peer mac
 *  @return         0 --success, otherwise fail
 */
static int
woal_tdls_add_sta_ioctl(moal_private * priv, u8 * peer)
{
	mlan_ioctl_req *ioctl_req = NULL;
	mlan_ds_misc_cfg *misc = NULL;
	int ret = 0;
	...
	misc = (mlan_ds_misc_cfg *) ioctl_req->pbuf;
	misc->sub_command = MLAN_OID_MISC_TDLS_ADD_STA;
	ioctl_req->req_id = MLAN_IOCTL_MISC_CFG;
	ioctl_req->action = MLAN_ACT_SET;
	memcpy(&misc->param.tdls_peer_mac, peer, ETH_ALEN);
	if (MLAN_STATUS_SUCCESS !=
	    woal_request_ioctl(priv, ioctl_req, MOAL_IOCTL_WAIT)) {
		ret = -EFAULT;
		goto done;
...
/**
 *  @brief TDLS teardown ioctl handler
 *
 *  @param priv     A pointer to moal_private structure
 *  @param peer     A pointer to peer mac
 *  @return         0 --success, otherwise fail
 */
static int
woal_tdls_teardown_ioctl(moal_private * priv, u8 * peer)
{
	mlan_ioctl_req *ioctl_req = NULL;
	mlan_ds_misc_cfg *misc = NULL;
	int ret = 0;
	...
	misc = (mlan_ds_misc_cfg *) ioctl_req->pbuf;
	misc->sub_command = MLAN_OID_MISC_TDLS_TEARDOWN;
	ioctl_req->req_id = MLAN_IOCTL_MISC_CFG;
	ioctl_req->action = MLAN_ACT_SET;
			...
			u8 * peer, enum nl80211_tdls_operation oper)
{
	...
	case NL80211_TDLS_ENABLE_LINK:
		PRINTM(MMSG, "NL80211_TDLS_ENABLE_LINK\n");
		action = WLAN_TDLS_ENABLE_LINK;
		break;
	case NL80211_TDLS_DISABLE_LINK:
		PRINTM(MMSG, "NL80211_TDLS_DISABLE_LINK\n");
	...
	ret = woal_tdls_oper_ioctl(priv, peer, action);
 ...
 */
static void
woal_tdls_add_ext_capab(struct sk_buff *skb)
{
	...
	*pos++ = 0x0;
	*pos++ = 0x0;
	*pos++ = WLAN_EXT_CAPA5_TDLS_ENABLED;	/* BIT37 */
}
...
woal_add_supported_rates_ie(moal_private * priv, struct sk_buff *skb)
{
	...
	t_u8 *pos;
	t_u8 rate_num = 0;
	if (priv->phandle->band == IEEE80211_BAND_2GHZ)
			       ...
			       t_u16 status_code, struct sk_buff *skb)
{
	...
	case WLAN_TDLS_SETUP_REQUEST:
		...
		woal_add_supported_rates_ie(priv, skb);
		woal_add_ext_supported_rates_ie(priv, skb);
		woal_tdls_add_ext_capab(skb);
		break;
	...
	case WLAN_TDLS_SETUP_RESPONSE:
		...
		HTcap->ieee_hdr.element_id = HT_CAPABILITY;
		HTcap->ieee_hdr.len =
			sizeof(IEEEtypes_HTCap_t) - sizeof(IEEEtypes_Header_t);
		HTcap->ht_cap.ht_cap_info = 0x0062;
		HTcap->ht_cap.ampdu_param = 0x3;
		HTcap->ht_cap.supported_mcs_set[0] = 0xff;
		...
		BSSCo->bss_co_2040.bss_co_2040_value = 0x0a;
		break;
	case WLAN_TDLS_SETUP_CONFIRM:
		HTInfo = (void *)skb_put(skb, sizeof(IEEEtypes_HTInfo_t));
		/* HT information */
		memset(HTInfo, 0, sizeof(IEEEtypes_HTInfo_t));
		HTInfo->ieee_hdr.element_id = HT_OPERATION;
		HTInfo->ieee_hdr.len =
			sizeof(IEEEtypes_HTInfo_t) - sizeof(IEEEtypes_Header_t);
		HTInfo->ht_info.pri_chan = priv->channel;
		HTInfo->ht_info.field3 = 0x5;
		HTInfo->ht_info.basic_mcs_set[0] = 0xff;
	default:
		break;
	}
			...
			size_t extra_ies_len)
{
	...
	PRINTM(MMSG, "wlan: TDLS peer=" MACSTR ", action=%d status_code=%d\n",
	       MAC2STR(peer), action_code, status_code);

	if (action_code == TDLS_SETUP_REQUEST)
		woal_tdls_add_sta_ioctl(priv, peer);
	...
	case TDLS_DISCOVERY_REQUEST:
		ret = woal_send_tdls_data_frame(wiphy, dev,
						peer, action_code, dialog_token,
						status_code, extra_ies,
						extra_ies_len);
		break;
	case TDLS_DISCOVERY_RESPONSE:
		ret = woal_send_tdls_action_frame(wiphy, dev,
						  peer, action_code,
	...
	if (action_code == TDLS_TEARDOWN || status_code) {
		woal_tdls_teardown_ioctl(priv, peer);
	}
	LEAVE();
	return ret;
...
woal_register_cfg80211(moal_private * priv)
{
	...
	*(unsigned long *)wdev_priv = (unsigned long)priv->phandle;
	...
	woal_cfg80211_init_wiphy(priv, MOAL_CMD_WAIT);
        /* For devices constrained by power; only enabling 1 Tx (Ant A)  
        while enabling 2 Rx (A+B). The second and third parameters are  
        bitfields for Tx and Rx*/  
        woal_cfg80211_set_antenna(priv->phandle->wiphy, 1, 3);  
